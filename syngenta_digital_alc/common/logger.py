...
class CommonLogger:
    def __init__(self):
        ...
        self.log_levels = {"INFO": 1, "WARN": 2, "ERROR": 3}

    def log(self, **kwargs):
        if (
            not os.getenv("RUN_MODE") == "unittest"
            and os.getenv("STAGE") == "local"
            and self.__should_log()
        ):
    ...
    def __get_current_log_setting(self):
        ...
        if not log_setting.isnumeric():
            return (
                self.log_levels[log_setting]
                if log_setting in self.log_levels
                else "ERROR"
            )
